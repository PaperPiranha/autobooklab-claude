export async function GET(req: Request) {
  ...
  if (!query) return Response.json({ photos: [], totalPages: 0 })

  const accessKey = process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY
  if (!accessKey) return Response.json({ photos: [], totalPages: 0 })
  ...
  try {
    ...
    if (!res.ok) return Response.json({ photos: [], totalPages: 0 })
    ...
    return Response.json({ photos, totalPages: data.total_pages ?? 1 })
  } catch {
    return Response.json({ photos: [], totalPages: 0 })
  }
}
